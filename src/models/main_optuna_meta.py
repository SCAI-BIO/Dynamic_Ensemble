...
def main(trial, config):
    ...
    # needs to be made categorical
    # meta data parameters
    config.meta_lookback = trial.suggest_categorical('lookback', [2,3,4])
    # meta LSTM parameters
    config.hs_dim =  trial.suggest_categorical('hs_dim', [4,8,16,32]) 
    ...
    # ens parameters
    config.lr_ens =  trial.suggest_categorical('lr_ens', [0.001,0.005,0.01,0.05,0.1])
    config.batch_size = trial.suggest_categorical('bs', [16,32,64,128])
    config.num_hidden_layers = trial.suggest_categorical('hl', [0,1,2])
    config.hs_ens_dim = trial.suggest_categorical('hs',[8,16,32])

    
    meta_csv = "/home/jbotz/Projects/aiolos_lstm/data/GER/Google_symptoms_de.csv"
    main_path = os.path.join(config.model_path, "results",config.data_name.partition("_")[0])
...
if __name__ == '__main__':
    ...
    config.save_path = os.path.join(config.save_path, config.exp_name, 
                                    config.data_name.partition("_")[0],config.type_pred)
    ...
    trial_path = save_path + '/optuna_trials_'+config.data_name.partition("_")[0]+"_meta_"+config.type_pred
