...
from train_meta_model import Train
from test import Test
import sys
from visualizations import *
sys.path.append('../')
from data.load_data import get_iters, load_dataset, load_data_AIOLOS, load_dataset_trees, load_dataset_ensemble, load_metadata
...
def main(config):
    ...
    meta_csv = "/home/jbotz/Projects/aiolos_lstm/data/GER/Google_symptoms_de.csv"
    ...
    config, dataloader, dataloader_test = load_dataset_ensemble(config, 
                                                                prediction_csv, 
                                                                scores_csv,meta_csv, 
                                                                meta=True, optuna=False, 
                                                                evaluation=False)
    config.type_pred="selection"
    if config.type_pred == "selection":
        trial_path_selection = os.path.join(config.model_path, "optuna_meta", 
                                            config.data_name.partition("_")[0], 
                                            config.type_pred, "optuna_trials_"
                                            +config.data_name.partition("_")[0]+
                                            "_meta_"+config.type_pred)
        storage_name = 'sqlite:///%s/database.db'%(trial_path_selection)
        study_name = "optuna_"+config.data_name.partition("_")[0]+"_meta_"+config.type_pred
        study = optuna.load_study(study_name=study_name, storage=storage_name)
        best_params=study.best_params
        #import ipdb; ipdb.set_trace()
        config.meta_lookback = best_params['lookback']
        config.lr = best_params['lr']
        config.hs_dim = best_params['hs_dim']
        config.num_layers = best_params['num_layers']
        config.drop_rate = best_params['drop_rate']

       
        config.lr_ens = best_params['lr_ens']
    ...
    if config.type_pred == "stacking":
        trial_path_stacking = os.path.join(config.model_path, 
                                           "optuna_meta", 
                                           config.data_name.partition("_")[0],
                                            config.type_pred, "optuna_trials_"+
                                            config.data_name.partition("_")[0]+
    ...
    scores_test_models = load_dataset_ensemble(config,prediction_csv,scores_csv,
                                               meta=False,optuna=False,evaluation=True)
    ...
    meta_mean_df.to_csv(save_path_output+"/scores_meta_mean.csv",index=False)
    meta_median_df.to_csv(save_path_output+"/scores_meta_median.csv")
    meta_std_df.to_csv(save_path_output+"/scores_meta_std.csv", index=False)
